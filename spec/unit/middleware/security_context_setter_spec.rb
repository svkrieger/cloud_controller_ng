    ...
    RSpec.describe SecurityContextSetter do
      ...
      describe '#call' do
        ...
        context 'when the rate limiter returns 429' do
          let(:app) { double(:app, call: [429, {}, 'a body']) }

          context 'when token is valid' do
            before do
              allow(VCAP::CloudController::SecurityContext).to receive(:valid_token?).and_return(true)
              allow(VCAP::CloudController::SecurityContext).to receive(:expired_token?).and_return(false)
            end
          ...
          context 'when token is expired, but valid otherwise' do
            before do
              allow(VCAP::CloudController::SecurityContext).to receive(:valid_token?).and_return(false)
              allow(VCAP::CloudController::SecurityContext).to receive(:expired_token?).and_return(true)
            end

            it 'changes the response to 401' do
              status, headers, body = middleware.call(env)
              expect(status).to eq(401)
              expect(headers).to eq({"Content-Length"=>"189", "Content-Type"=>"application/json"})
